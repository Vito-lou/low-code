...
function setEndOfContenteditable(contentEditableElement) {
    ...
    const selection = shared_1.globalThisPolyfill.getSelection();
    selection.removeAllRanges();
    selection.addRange(range);
}
function createCaretCache(el) {
    const currentSelection = shared_1.globalThisPolyfill.getSelection();
    if (currentSelection.containsNode(el))
        return;
    const ranges = getAllRanges(currentSelection);
    return (offset = 0) => {
        const sel = shared_1.globalThisPolyfill.getSelection();
...
const useContentEditableEffect = (engine) => {
    const globalState = {
    ...
    function onInputHandler(event) {
        ...
        if (node) {
            ...
            const handler = () => {
                ...
                path_1.Path.setIn(node.props, this.getAttribute(engine.props.contentEditableAttrName), target === null || target === void 0 ? void 0 : target.textContent);
    ...
    function onPastHandler(event) {
        ...
        const selObj = shared_1.globalThisPolyfill.getSelection();
        const target = event.target;
        const selRange = selObj.getRangeAt(0);
        const restore = createCaretCache(target);
        selRange.deleteContents();
        selRange.insertNode(document.createTextNode(text));
        path_1.Path.setIn(node.props, this.getAttribute(engine.props.contentEditableAttrName), target.textContent);
        restore(text.length);
    }
    ...
    engine.subscribeTo(events_1.MouseClickEvent, (event) => {
        ...
        const editableElement = (_a = target === null || target === void 0 ? void 0 : target.closest) === null || _a === void 0 ? void 0 : _a.call(target, `*[${engine.props.contentEditableAttrName}]`);
        if (editableElement &&
    ...
    engine.subscribeTo(events_1.MouseDoubleClickEvent, (event) => {
        var _a;
        const target = event.data.target;
        const editableElement = (_a = target === null || target === void 0 ? void 0 : target.closest) === null || _a === void 0 ? void 0 : _a.call(target, `*[${engine.props.contentEditableAttrName}]`);
